...
async def websocket_endpoint(
    ...
    await broker.connect_client(client_id, websocket)
                ...
                if action == "subscribe":
                    ...
                    if topic:
                        if security_validator.authorize(token, "subscribe", topic):
                            ...
                            await websocket.send_json({"error": f"Forbidden: No subscribe ACL for '{topic}'"})
                    ...
                    if topic and payload is not None:
                        if security_validator.authorize(token, "publish", topic):
                            await message_service.publish(MessageCreate(topic=topic, payload=payload))
                        else:
                            await websocket.send_json({"error": f"Forbidden: No publish ACL for '{topic}'"})
                    ...
                    if token == "admin-token-999":
                        ...
                        await websocket.send_json({"error": "Forbidden: Admin token required for logs"})
